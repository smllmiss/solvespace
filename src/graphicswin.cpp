...
{ 1, N_("&Unselect All"),               Command::UNSELECT_ALL,     '\e',    KN, mEdit  },
...
{ 1,  NULL,                             Command::NONE,             0,       KN, NULL   },
{ 1, N_("Dimensions in &Inches"),       Command::UNITS_INCHES,     0,       KR, mView  },
{ 1, N_("Dimensions in &Millimeters"),  Command::UNITS_MM,         0,       KR, mView  },
{ 1,  NULL,                             Command::NONE,             0,       KN, NULL   },
...
std::string GraphicsWindow::AcceleratorForCommand(Command cmd) {
    int accel = 0;
    for(int i = 0; Menu[i].level >= 0; i++) {
        if(cmd == Menu[i].cmd) {
            accel = Menu[i].accel;
    ...
    accel &= ~(CTRL_MASK | SHIFT_MASK);
    if(accel & FN_MASK) {
        label += ssprintf("F%d", accel & ~FN_MASK);
    } else if(accel == '\t') {
        label += "Tab";
    } else if(accel == ' ') {
        label += "Space";
...
void GraphicsWindow::PopulateMainMenu() {
    Platform::MenuBarRef mainMenu = Platform::CreateMenuBar();
    ...
    for(int i = 0; Menu[i].level >= 0; i++) {
        ...
        if(Menu[i].level == 0) {
            ...
            if(Menu[i].accel != 0) {
                ...
                if(Menu[i].accel & SHIFT_MASK) {
                    accel.shiftDown = true;
                }
                if(Menu[i].accel & CTRL_MASK) {
                    accel.controlDown = true;
                }
                if(Menu[i].accel & FN_MASK) {
                    accel.key = Platform::KeyboardEvent::Key::FUNCTION;
                    accel.num = Menu[i].accel & 0xff;
                } else {
                    accel.key = Platform::KeyboardEvent::Key::CHARACTER;
                    accel.chr = (char)(Menu[i].accel & 0xff);
                }
